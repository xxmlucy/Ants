    ...
    def buff(self):
        ...
        if self.is_container:
            if self.ant_contained:
                if not self.ant_contained.buffed:
                    self.ant_contained.damage*=2
                    self.ant_contained.buffed = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def nearest_bee_finder(place):
            if place.is_hive:
                return None 
            if self.min_range<=self.location<=self.max_range:
                if place.bees:
                    return random_bee(place.bees)
            self.location += 1
            return nearest_bee_finder(place.entrance)
        return nearest_bee_finder(current_place)
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        elif self.chew_countdown == 0:
            if self.place.bees:
                self.chew_countdown = self.chew_duration
                kill_bee = random_bee(self.place.bees)
                kill_bee.reduce_health(kill_bee.health)
# END Problem 7
...
class Bee(Insect):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    is_waterproof = True
    def __init__(self, health=1, slowness=0, scareness = 0):
        self.slowness = slowness
        self.scareness = scareness
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=2, has_queen=False):
